<template>
    <div class="register">
        <div class="top">
            <div class="title">{{ detail.hospitalInfo?.hospital?.hosname }}</div>
            <div class="level">
                <svg t="1730450598160" class="icon" viewBox="0 0 1024 1024" version="1.1"
                    xmlns="http://www.w3.org/2000/svg" p-id="4288" width="16" height="16">
                    <path
                    ...
                    <path
                        ...
                        fill="#7dc5eb" p-id="4291"></path>
                </svg>
                {{ detail.hospitalInfo.hospital?.param?.hostypeString }}
            </div>
        </div>
        <div class="content">
            <div class="left">
                <img :src="`data:image/jpeg;base64,${detail.hospitalInfo?.hospital?.logoData}`" alt="">
            </div>
            <div class="right">
                <div class="rule1">挂号规则</div>
                <div class="time">预约周期:10天 放号时间:{{ detail.hospitalInfo?.bookingRule?.releaseTime }} 停挂时间:{{
                    detail.hospitalInfo?.bookingRule?.stopTime }}</div>
                <div class="address">
                    具体地址:{{ detail.hospitalInfo?.hospital?.address }}
                </div>
                <div class="route">
                    具体路线:{{ detail.hospitalInfo?.hospital?.route }}
                </div>
                <div class="releasetime">
                    退号时间:就诊前以工作日{{ detail.hospitalInfo?.bookingRule?.quitTime }}前取消
                </div>
                <div class="rule2">预约挂号规则</div>
                <div class="ruleInfo" v-for="(item, index) in detail.hospitalInfo?.bookingRule?.rule" :key="index">
                    {{ item }}
                </div>
            </div>
        </div>
